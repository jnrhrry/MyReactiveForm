    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val passwordConfirmationStream = Observable.merge(
            RxTextView.textChanges(ed_password)
                .map { password -> password.toString() != ed_confirm_password.text.toString() },
            RxTextView.textChanges(ed_confirm_password)
                .map { confirmPassword -> confirmPassword.toString() != ed_password.text.toString() }
        )
        passwordConfirmationStream.subscribe{showPasswordMinimalAlert(it)}
